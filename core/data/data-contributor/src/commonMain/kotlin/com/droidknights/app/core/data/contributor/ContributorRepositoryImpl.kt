...
internal class ContributorRepositoryImpl(
    ...
    private fun buildYearGroups(responses: List<ContributionYearResponse>): List<Pair<Int, List<ContributionYearResponse>>> {
        val yearToIds = mutableMapOf<Int, MutableSet<ContributionYearResponse>>()

        responses.forEach { response ->
            response.years.forEach { year ->
                yearToIds.getOrPut(year) { mutableSetOf() }
                    .add(response) // year 기준으로 그룹화
            }
        }
